...
//          + " AND (:#{#param.birthDate} IS NULL OR c.birthDate <= :#{#param.birthDateTo}) "
  ...
  @Query(value = "SELECT c.id AS id, c.code AS code, n.tenNhomKhachHang AS tenNhomKhachHang," +
          " c.tenKhachHang AS tenKhachHang, c.created AS created, c.soDienThoai AS soDienThoai," +
          " c.MappingStoreId as mappingStoreId, c.zaloId AS zaloId, c.barcode AS barcode" +
          " FROM KhachHangs c" +
          " JOIN NhomKhachHangs n ON c.id = n.id" +
          " WHERE 1=1 "
